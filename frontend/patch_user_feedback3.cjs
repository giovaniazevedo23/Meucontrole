...
// 1. Remove icon from "Exportar Histórico (Arquivados)"
app = app.replace('📥 Exportar Histórico (Arquivados)', 'Exportar Histórico (Arquivados)');

// 2. Remove icon from "Rastreio"
app = app.replace('📍 Rastreio', 'Rastreio');

// 3. Replace "Clique para emitir NF-e"
app = app.replace('Clique para emitir NF-e', 'Selecionar NF-e');

// 4. Remove emojis from NFE Upload
app = app.replace("isEmitindoNfe ? '⏳ Fazendo Upload...' : '⬆️ Concluir Upload'", "isEmitindoNfe ? 'Fazendo Upload...' : 'Concluir Upload'");

// 5. Update search logic in sistema tab
const startSearch = app.indexOf('{(() => {');
const endSearchStr = ').map(deal => {';
const endSearch = app.indexOf(endSearchStr, startSearch) + endSearchStr.length;

if (startSearch > -1 && app.indexOf(endSearchStr, startSearch) > -1) {
  // Check if we are at the right block (sistemaSearch)
  const block = app.substring(startSearch, endSearch);
  if (block.includes('sistemaSearch.toLowerCase()')) {
    ...
    app = app.substring(0, startSearch) + searchLogicNew + app.substring(endSearch);
  }
}


// 6. Expand systemDetailsModal
const startModal = app.indexOf('{systemDetailsModal && (');
const endModalStr = '</div>\r\n        </div>\r\n      )}';
let endModal = app.indexOf(endModalStr, startModal);
if (endModal === -1) endModal = app.indexOf('</div>\n        </div>\n      )}', startModal);
endModal += endModalStr.length;
...
if (startModal > -1 && endModal > -1) {
  app = app.substring(0, startModal) + detailsModalNew + app.substring(endModal);
} else {
  console.log("Could not find modal bounds!");
}

fs.writeFileSync('src/App.jsx', app, 'utf8');
console.log('All patches applied via index splicing.');
